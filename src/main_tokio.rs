...
async fn main() {
    ...
    let (response_sender, response_receiver) =
        channel::<(SocketAddr, Instrumentation, DnsMessage)>();
    ...
    loop {
        ...
        tokio::spawn(async move {
            let remote_answer = if filter_query(filter, &query, verbosity) {
                ...
                if let Ok(result) =
                    query_remote_dns_server_doh(DEFAULT_DOH_DNS_RESOLVER, query).await
                {
...
fn filter_query(filter: Arc<Mutex<Filter>>, query: &DnsMessage, verbosity: u8) -> bool {
